...
In addition to government bond yields and inflation swaps, we consider the nominal euro-dollar exchange rate \rev{in price notation, that is, euros per US dollar} (\texttt{FX}, in percentage changes) and the Euro Stoxx 50 index as a stock market indicator (\texttt{Stocks}, in percentage changes). \rev{An increase in \texttt{FX} therefore denotes a depreciation of the euro against the dollar.} We also include the EA composite indicator of systemic stress \citep[\texttt{CISS},][]{kremer2012CISS}. This index captures 15 indicators of stress in financial markets, and thus should be able to capture financial stability issues associated with monetary policy shocks. \rev{The yield and swap curve factors, the exchange rate and the equity index enter in first differences to ensure stationarity. Two variables are exceptions: the \texttt{CISS}, which is bounded between zero and one by construction and therefore enters in levels, and the policy surprise itself, which is already a measured innovation and is zero in weeks without a Governing Council announcement.} %That is, the one Euro per US-dollar, such that a decrease denotes an appreciation of the Euro.
...
%We repeat this exercise in \autoref{eq:univRegr} with the corresponding inflation swap factors and report the results in \autoref{tab:IE_swaps}.  Compared to the yield curve factors, we observe a statistically significant reaction due to ECB surprises only for the slope factor and  the monetary policy interaction part only for the level. That is, the respective level of inflation expectations decreases after the Fed produces a positive surprise, though the effect is rather negligible. This points toward imported disinflation dynamics from the US that amplify the domestic effect of monetary policy. Given the results from the curvature part of the expectation term structure, we neither see pronounced reactions to ECB surprises nor substantial interaction effects. While this univariate analysis suffers from several shortcomings (such as the low explanatory power and the focus on contemporaneous relations between yield curve dynamics and monetary policy) and thus serves only as a motivation for a unified analysis, it motivates further explorations. An example is the omission of effects that simultaneously alter the shape of the yield and swap curves. Moreover, a contemporaneous reaction of yields and inflation swaps might be likely and potentially exhibits offsetting dynamics. To gain a deeper understanding,  we will thus  provide evidence based on our ST-VAR  in the subsequent sections.
