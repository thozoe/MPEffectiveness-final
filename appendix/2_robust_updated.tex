...
and increasing in maturity from ten years out; the posterior probability of a positive gap at thirty years is at least $0.88$ in seven of the eight specifications and $0.81$ in the eighth. Its magnitude, however, varies by a factor of about two and a half across specifications, and we accordingly make no claim about the size of the effect in the main text.
...
\autoref{tab:robrot} states the rotation directly. For each maturity it records the highest percentile of the Fed's stance at which the response is still significantly positive; reading down a column traces the rotation, since a response that survives at every percentile at short maturities but fails beyond the fiftieth or sixtieth at long maturities is precisely a curve that pivots rather than shifts. In no specification does the long-end response turn significantly negative at any point of the stance, so the rotation is a loss of significance rather than a sign reversal.

\begin{table}[!htpb]
\caption{The rotation:significance.\label{tab:robrot}}
\centering \footnotesize
\resizebox{\textwidth}{!}{\input{tables/tab_rob_rotation}}
\caption*{\footnotesize \textit{Notes:} Entries are percentiles of the Fed's stance distribution: the highest percentile at which the sixteenth posterior quantile of the response is still above zero. A value of $100$ means the response remains significantly positive across the entire stance distribution. In no specification does the eighty-fourth quantile fall below zero at any maturity or percentile, so the response never becomes significantly negative.}
\end{table}
...
calibrated neutral stance $\mu$ in percentage points of the Fed's stance. The open red circle marks
