...
\title{\huge{Hawks vs. Doves: The Fed's stance and the Term Structure of ECB Policy Transmission}} 
...
\begin{abstract} 
\noindent 
%% [2026-07-27] Condensed from 377 to about 190 words. Proposed version in \sug{} (blue);
%% the superseded abstract is recoverable from git at commit 030887b.
\sug{Monetary policies diverge across countries, yet models of domestic transmission typically treat the foreign policy environment as irrelevant. We ask whether the transmission of ECB policy to euro area financial markets depends on the stance of the Federal Reserve. We estimate a Bayesian smooth transition VAR on weekly data for 2005--2025 in which the dynamic coefficients vary with the deviation of the federal funds rate from the US natural rate of interest, and trace the response of the euro area term structure to high-frequency ECB policy surprises across the full range of the Fed's stance. The Fed's stance does not shift the pass-through of an ECB surprise up or down: it \textit{rotates} it. Under an accommodative Fed, a contractionary surprise raises yields by a similar amount all along the curve; under a restrictive Fed, the short end responds more strongly while the long end does not respond at all. State dependence in ECB transmission is thus a phenomenon of the long end. At the short end the composition changes instead: inflation compensation falls about three times more when the Fed is restrictive, so that real short-term financing conditions tighten by appreciably more than nominal yields suggest. The sign and the maturity profile are robust to how the stance is measured, how the surprise is identified, and how the regime boundary is calibrated.}
\end{abstract}
...
% Input blocks
\input{1_introduction_updated}

\input{2_framework_updated}

\input{3_data_updated}

\input{4_results_updated}

\input{5_conclusio_updated}
...
\input{appendix/0_Data_updated}
\newpage
\section{Further empirical results}\label{app:A}
\input{appendix/1_add_res_updated}

\clearpage
\section{Robustness Checks}\label{app:rob}
\input{appendix/2_robust_updated}

\clearpage
\section{Technical appendix}\label{app:B}
\input{appendix/2_technical_updated}
