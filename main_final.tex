...
\begin{abstract} 
\noindent 
Monetary policies diverge across countries, yet models of domestic monetary-policy transmission often abstract from the foreign policy environment or capture its effects only indirectly. In this paper, we analyze \rev{whether} the transmission of ECB policy to euro area financial markets depends on the stance of the Federal Reserve. Using weekly data from 2005 to 2025, we estimate a Bayesian smooth-transition VAR in which dynamic coefficients vary with the U.S. monetary-policy stance. We then trace the response of the euro-area term structure to high-frequency ECB monetary-policy surprises across the full range of the Fed's policy stance. The Fed's stance does not shift the pass-through of an ECB surprise up or down: it rotates it. Under an accommodative Fed, a contractionary surprise raises yields by a similar amount all along the curve. Under a restrictive Fed, the short end responds more strongly while the long end does not respond at all. State dependence in ECB transmission is thus a phenomenon of the long end. At the short end the composition changes instead as inflation compensation falls about three times more when the Fed is restrictive, so that real short-term financing conditions tighten by appreciably more than nominal yields suggest. %The sign and the maturity profile are robust to how the stance is measured, how the surprise is identified, and how the regime boundary is calibrated.
\end{abstract}
...
% Input blocks
\input{0_introduction_updated}

\input{1_literature_updated}
