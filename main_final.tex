...
\begin{abstract} 
\noindent 
The secular increase in globalization led to a substantial increase in the interconnectedness of global financial markets. This has important implications for the conduct of monetary policy, as central bank policies may diverge across countries, potentially affecting key transmission channels of domestic policy actions. In this paper, we use a non-linear multivariate time series model to shed light on how the US monetary policy stance affects the conduct of monetary policy in the euro area. We assume that the dynamic coefficients implicitly depend on a measure of the Federal Reserve's policy stance through a smooth transition function. This assumption allows us to examine how the dynamic responses of financial market quantities such as government bond yields and inflation swaps to euro area monetary policy shocks change with the US policy stance. \rev{Tracing the response of euro area yields to an ECB tightening across the full range of the Fed's stance, we find that the transmission of ECB policy through financial markets depends systematically and monotonically on that stance: the reaction is strongest when the Fed is accommodative and fades, or reverses at long maturities, as the Fed turns restrictive. This state dependence is robust to controlling for global forces (oil prices, the US dollar, and global risk aversion), is distinct from the euro area business cycle rather than an artifact of the effective lower bound period, and is estimated on a sample running through the end of 2025 whose post-pandemic tightening cycle sharpens its identification.}
\sug{[Suggested calibration for the revision. The preliminary reduced-form evidence (\autoref{sec:passthrough}) robustly establishes that ECB surprises transmit through the term structure, but at weekly frequency it is too imprecise to identify \emph{state dependence}; that result is a finding of the structural model, not a model-free stylized fact. We therefore suggest attributing it explicitly to the model, e.g. ``Using a Bayesian smooth-transition VAR, we show that the transmission $\dots$ depends systematically on the Federal Reserve's stance,'' rather than phrasing that implies the pattern is already visible in simple regressions.]}
\end{abstract}
...
%% [TEMP 2026-07-22] review-only section documenting the Timing-factor change.
%% DELETE this \input and the file TEMP_timing_changes.tex before circulating the draft.
\input{TEMP_timing_changes}
